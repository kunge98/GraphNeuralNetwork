...
class trainer():
    ...
    def test(model, save_path_resume, device, dataloader, scaler, model_name, save=True, **kwargs):
        ...
        for i in range(12):
            ...
            pred    = yhat[:,:,i]
            real    = realy[:,:,i]
            if kwargs['dataset_name'] == 'PEMS04' or kwargs['dataset_name'] == 'PeMS_08':  # traffic flow dataset follows mae metric used in ASTGNN.
                mae     = mean_absolute_error(pred.cpu().numpy(), real.cpu().numpy())
                rmse    = masked_rmse(pred, real, 0.0).item()
                mape    = masked_mape(pred, real, 0.0).item()
                log     = 'Evaluate best mymodel on test data for horizon {:d}, Test MAE: {:.4f}, Test RMSE: {:.4f}, Test MAPE: {:.4f}'
