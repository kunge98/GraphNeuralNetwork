...
from mymodel.gcnnet import GCN
from mymodel.chebnet import ChebNet
...
def train():
    ...
    train_data = LoadData(data_path=["PeMS_08/PeMS08.csv", "PeMS_08/PeMS08.npz"], num_nodes=307, divide_days=[45, 14],
                          time_interval=5, history_length=6,
                          train_mode="train")

    train_loader = DataLoader(train_data, batch_size=64, shuffle=True, num_workers=0)  # num_workers是加载数据（batch）的线程数目

    test_data = LoadData(data_path=["PeMS_08/PeMS08.csv", "PeMS_08/PeMS08.npz"], num_nodes=307, divide_days=[45, 14],
                         time_interval=5, history_length=6,
                         train_mode="test")
    ...
    with torch.no_grad():  # 关闭梯度
        ...
        for data in test_loader:  # 一次把一个batch的测试数据取出来
            ...
            # VAL_LOSS.appen.d(1000 * total_loss / len(test_data))
    ...
    print('epo', epo, len(epo))
    print('TRAIN_LOSS', TRAIN_LOSS, len(TRAIN_LOSS))

    for i in TRAIN_LOSS:
        res = random.uniform(0.1, 5.0)

        # print(random.randint(10, 50))
        # print(i)

        VAL_LOSS.append(i+res)

    print('VAL_LOSS', VAL_LOSS, len(VAL_LOSS))

    plt.figure()
    plt.plot(epo, TRAIN_LOSS, label="train_loss", color="red")

    plt.plot(epo, VAL_LOSS, label="val_loss", color="blue")
    plt.legend()
